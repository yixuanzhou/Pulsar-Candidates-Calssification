...
dt = DecisionTreeClassifier(max_depth=22, min_samples_split=2, min_samples_leaf=2, random_state=95)
...
for scoring in ('f1', 'roc_auc', 'precision', 'recall'):
    res[scoring] = cross_val_score(clf_dt, X_test, y_test, cv=cv, scoring=scoring, n_jobs=-1)
print(res['f1'].mean(), res['roc_auc'].mean(), res['precision'].mean(), res['recall'].mean())

# Final result for training set and test set
print(evaluation.test_score(clf_dt, X_train, y_train))
print(evaluation.test_score(clf_dt, X_test, y_test))

# Plot confusion
cmyy = metrics.confusion_matrix(y_test, y_pred)
metcm = [cmyy]
plot_confusion_matrix(np.mean(metcm,axis=0), ['RFI','PULSAR'], cmap=plt.cm.RdPu, title='Decision Tree', colmap=True)
plt.show()